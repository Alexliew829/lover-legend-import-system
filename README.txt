Lover Legend 进口成本与库存系统 — 正式版 V4.27 Stable
...
- 旧 V4.20/V4.25 前端没有 V4.27 clientVersion + schemaVersion，因此后端会阻止其 Push。
- 新前端即使某对象缺少字段，后端也先与 Sheet 原记录按 ID 合并，避免缺字段被写成空白。
- 日常同步不执行格式化，减少 Apps Script API 调用。
- V4.27 第一次启动固定执行 Full Pull；未完成首次 Pull 前，前端不会建立 dirty 队列，后端也不会接受 Push。
