...
async function commitSalesCorrectionBatchToCloudV108(payload) {
  await flushCloudQueueStrictV83(); const config=getCloudConfig(); setCloudState("syncing");
  try { const data=await callGoogleApi({action:"commitSalesCorrectionBatchV108",clientVersion:APP_VERSION,schemaVersion:CLOUD_SCHEMA_VERSION,baseRevision:Number(config.revision)||0,bootstrapToken:String(config.bootstrapToken||""),bootstrapRevision:Number(config.bootstrapRevision)||0,updatedBy:"System V10.8 Stable",...payload});
...
window.commitSalesCorrectionBatchToCloudV108=commitSalesCorrectionBatchToCloudV108;
