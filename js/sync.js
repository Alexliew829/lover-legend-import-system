...
// V36.1: remember whether a real unsynced queue already existed before any
...
function setupCloudSync() {
  ...
  // V36.1 Local-First: when a valid V32.5-style bootstrap and cached core data
...
async function commitSalesCorrectionBatchToCloudV110(payload) {
  ...
  try { const data=await callGoogleApi({action:"commitSalesCorrectionBatchV110",clientVersion:APP_VERSION,schemaVersion:CLOUD_SCHEMA_VERSION,baseRevision:Number(config.revision)||0,bootstrapToken:String(config.bootstrapToken||""),bootstrapRevision:Number(config.bootstrapRevision)||0,updatedBy:"System V36.1 Stable",...payload});
...
async function runCloudSync() {
  ...
  try {
    ...
    // V36.1: if the queue was clean before this page loaded but became dirty while
    ...
    // V36.1: after one successful bootstrap, read-only revision checks stay in the
    ...
    // V36.1: a bootstrap record from an older cached frontend may be complete but
...
async function pullLatestSnapshot(forceBootstrap = false) {
  ...
  if (data.unchanged) {
    ...
    saveCloudConfig(config);
    // V36.1: even an unchanged Pull can return a fresh bootstrap token. Persist
...
function retryPendingMinimumPriceV345(){
  ...
  Promise.resolve(updateProductMinimumPriceFast(pending.productId,pending.minimumPrice,pending.updatedAt,pending.minimumPriceManual))
    .catch(error=>console.warn("V36.1 pending minimum-price retry kept for next sync",error))
...
async function updateProductMinimumPriceFast(productId, minimumPrice, updatedAt, minimumPriceManual = true, retryCountV341 = 0) {
  ...
  // V36.1: keep this single-product change authoritative locally as well.
  ...
  try {
    ...
    console.warn("V36.1 minimum-price local refresh skipped", localErrorV341);
  }

  setMinimumPricePendingV345(null);
  renderCloudMeta(config);
  setCloudState("synced");
  window.setTimeout(()=>{try{if(typeof refreshSystemViewsAfterSync==="function")refreshSystemViewsAfterSync()}catch(refreshErrorV343){console.warn("V36.1 deferred minimum-price refresh skipped",refreshErrorV343)}},0);
  return data;
}
...
async function updatePromotionSettingsFastV185(promotion) {
  ...
  if (data.conflict) {
    // V36.1: preserve the cloud's currently active promotion. A failed update is
    ...
    config.bootstrapRevision = Number(data.revision) || Number(config.bootstrapRevision) || 0;
    saveCloudConfig(config);
    try { await pullLatestSnapshot(false); } catch (pullErrorV350) { console.warn("V36.1 promotion conflict refresh skipped", pullErrorV350); }
...
async function pushPendingSnapshot(queue, retryCount = 0) {
  ...
  const data = await callGoogleApi({
    ...
    updatedBy:"System V36.1 Stable", collections,
...
function sanitizeLegacySettingsV323(settings = {}, products = []) {
  ...
  // V36.1: promotion is live business state, not legacy residue. Preserve a valid
...
function applyRemoteData(data) {
  ...
  try {
    // V36.1: keep the proven V32.5 Pull/apply path, but promotionV183 is now
    ...
    // V36.1: direct cloud writes must invalidate the prepared inventory join.
  ...
  // V36.1: a cloud Pull is read-only. Do not auto-repair/write cost snapshots
