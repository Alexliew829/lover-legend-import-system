...
async function commitSalesCorrectionBatchToCloudV110(payload) {
  ...
  try { const data=await callGoogleApi({action:"commitSalesCorrectionBatchV110",clientVersion:APP_VERSION,schemaVersion:CLOUD_SCHEMA_VERSION,baseRevision:Number(config.revision)||0,bootstrapToken:String(config.bootstrapToken||""),bootstrapRevision:Number(config.bootstrapRevision)||0,updatedBy:"System V41.4 Stable",...payload});
...
function repairLocalBsCanonicalCacheV365() {
  try {
    ...
    console.warn("V41.4 local BS canonical cache repair skipped", error);
    return false;
  }
}
...
async function pullLatestSnapshot(forceBootstrap = false) {
  ...
  if (data.unchanged) {
    ...
    saveCloudConfig(config);
    // V40.3: keep the proven main revision path independent from Promotion Light Sync.
  ...
  if (hasBsLegacyIdsV364(data.products)) {
    const repair = await callGoogleApi({
      ...
      bootstrapRevision:Number(data.revision)||0, updatedBy:"System V41.4 Stable"
    });
  ...
  // V40.3: a lightweight promotion response may have advanced this device while
...
function retryPendingMinimumPriceV345(){
  ...
  Promise.resolve(updateProductMinimumPriceFast(pending.productId,pending.minimumPrice,pending.updatedAt,pending.minimumPriceManual,0,pending.historyMetaV374||null,Boolean(pending.preserveInitialMinimumPriceV409)))
    .catch(error=>console.warn("V41.4 pending minimum-price retry kept for next sync",error))
    .finally(()=>{minimumPricePendingRetryBusyV345=false;});
}
window.retryPendingMinimumPriceV345=retryPendingMinimumPriceV345;
async function updateProductMinimumPriceFast(productId, minimumPrice, updatedAt, minimumPriceManual = true, retryCountV341 = 0, historyMetaV374 = null, preserveInitialMinimumPriceV409 = false) {
  ...
  setMinimumPricePendingV345({productId:String(productId||""),minimumPrice:Number(minimumPrice)||0,minimumPriceManual:Boolean(minimumPriceManual),updatedAt:String(updatedAt||new Date().toISOString()),historyMetaV374:historyMetaV374||null,preserveInitialMinimumPriceV409:Boolean(preserveInitialMinimumPriceV409)});
  setCloudState("syncing");
  ...
  const data = await callGoogleApi({
    ...
    preserveInitialMinimumPriceV409: Boolean(preserveInitialMinimumPriceV409)
  });
  ...
  if (data.conflict) {
    ...
    if (retryCountV341 < 2) {
      ...
      return updateProductMinimumPriceFast(productId, minimumPrice, updatedAt, minimumPriceManual, retryCountV341 + 1, historyMetaV374, preserveInitialMinimumPriceV409);
    }
    reapplyPendingMinimumPriceLocalV345();
    setCloudState("syncing");
  ...
  try {
    ...
    // V41.4 root-cause fix: Number(null)===0. V38.1 therefore overwrote a valid
    ...
    console.warn("V41.4 minimum-price local refresh skipped", localErrorV341);
  }

  setMinimumPricePendingV345(null);
  renderCloudMeta(config);
  setCloudState("synced");
  window.setTimeout(()=>{try{if(typeof refreshSystemViewsAfterSync==="function")refreshSystemViewsAfterSync()}catch(refreshErrorV343){console.warn("V41.4 deferred minimum-price refresh skipped",refreshErrorV343)}},0);
  return data;
}
...
async function reconcilePromotionWriteV374(requestedPromotion, attempts = 2) {
  for (let indexV373 = 0; indexV373 < attempts; indexV373 += 1) {
    ...
    try {
      ...
      const remotePromotionV373 = stateV373?.promotionV183 && stateV373.promotionV183.active === true ? stateV373.promotionV183 : null;
      if (normalizePromotionStateForCompareV372(requestedPromotion) === normalizePromotionStateForCompareV372(remotePromotionV373)) {
        return stateV373;
      }
    } catch (errorV373) {
      console.warn("V41.4 promotion timeout reconcile skipped", errorV373);
    }
  }
  return null;
}
...
async function updatePromotionSettingsFastV185(promotion, retryCountV374 = 0, forceFreshCredentialV398 = false) {
  ...
  setCloudState("syncing");
  try {
    // V40.3: normal Promotion writes use the already-valid bootstrap credential first.
    ...
    if (/timeout|connection failed/i.test(messageV374)) {
      // V40.3: a timeout may mean the server already committed the write.
...
function normalizePromotionStateForCompareV372(value) {
  ...
  return JSON.stringify({
    ...
    promotionScope:p.promotionScope === "selected" ? "selected" : "exclude",
    ...
    marginOverrides:Object.fromEntries(Object.entries(p.marginOverrides && typeof p.marginOverrides === "object" ? p.marginOverrides : {}).sort(([a],[b])=>a.localeCompare(b))),
...
function getPromotionLightPollIntervalV375() {
  ...
  // V41.4 mobile promotion control uses the same proven Promotion Light Sync path
...
async function pollPromotionStateLightV372(force = false) {
  ...
  try {
    ...
    // V40.3: if a Start/Update/Close began after this poll was sent, this response
    ...
    if (changedV372 || promotionRevisionAdvancedV376) {
      ...
      } catch (error) { console.warn("V41.4 promotion light repaint skipped", error); }
    }
    ...
    console.warn("V41.4 promotion light sync skipped", error);
    return false;
  } finally {
...
async function pushPendingSnapshot(queue, retryCount = 0) {
  ...
  const data = await callGoogleApi({
    ...
    updatedBy:"System V41.4 Stable", collections,
  ...
  if (promo && promo.active === true) {
    ...
    if (Number.isFinite(commissionRate) && commissionRate >= 0 && commissionRate < 100 &&
      ...
      out.promotionV183 = {
        ...
        promotionScope: promo.promotionScope === "selected" ? "selected" : "exclude",
        selectedProductIds: [...new Set((Array.isArray(promo.selectedProductIds) ? promo.selectedProductIds : [])
          .map(id => String(id || "").trim().toUpperCase())
          .filter(id => id && validIds.has(id)))],
        ...
        marginOverrides: Object.fromEntries(Object.entries(promo.marginOverrides && typeof promo.marginOverrides === "object" ? promo.marginOverrides : {})
          .map(([id, rate]) => [String(id || "").trim().toUpperCase(), Number(rate)])
          .filter(([id, rate]) => id && validIds.has(id) && Number.isFinite(rate) && rate >= -99.9 && rate < 100 && 1 - commissionRate / 100 - rate / 100 > 0)),
...
function mergeSnapshots(remote, local, queue) {
  ...
  // V40.3: dedicated cloud writers own these live pricing/promotion keys. During a
...
async function updateInitialMinimumPriceFastV376(productId, initialMinimumPrice, retryCount=0){
  if(!isCloudWriteCredentialCurrentV341()) await pullLatestSnapshot(false);
  const config=getCloudConfig();
  const data=await callGoogleApi({action:"updateInitialMinimumPriceV376",clientVersion:APP_VERSION,schemaVersion:CLOUD_SCHEMA_VERSION,baseRevision:Number(config.revision)||0,bootstrapToken:String(config.bootstrapToken||""),bootstrapRevision:Number(config.bootstrapRevision)||0,updatedBy:"System V41.4 Stable",productId:String(productId||"").trim(),initialMinimumPrice:Number(initialMinimumPrice)});
...
async function restoreInitialMinimumPricesFastV376(retryCount=0){
  ...
  const data=await callGoogleApi({action:"restoreInitialMinimumPricesV376",clientVersion:APP_VERSION,schemaVersion:CLOUD_SCHEMA_VERSION,baseRevision:Number(config.revision)||0,bootstrapToken:String(config.bootstrapToken||""),bootstrapRevision:Number(config.bootstrapRevision)||0,updatedBy:"System V41.4 Stable"});
