...
const CLOUD_BOOTSTRAP_KEY = "importSystemCloudBootstrapV4263";
...
async function runCloudSync() {
  ...
  try {
    ...
    // V4.26.3 hard bootstrap: this version's first successful sync is ALWAYS a full Pull.
...
async function pushPendingSnapshot(queue, retryCount = 0) {
  ...
  const data = await callGoogleApi({
    ...
    bootstrapRevision: Number(config.bootstrapRevision) || 0,
    updatedBy: "System V4.26.3 Stable",
