...
async function commitSalesCorrectionBatchToCloudV110(payload) {
  ...
  try { const data=await callGoogleApi({action:"commitSalesCorrectionBatchV110",clientVersion:APP_VERSION,schemaVersion:CLOUD_SCHEMA_VERSION,baseRevision:Number(config.revision)||0,bootstrapToken:String(config.bootstrapToken||""),bootstrapRevision:Number(config.bootstrapRevision)||0,updatedBy:"System V37.9 Stable",...payload});
...
function repairLocalBsCanonicalCacheV365() {
  try {
    ...
    console.warn("V37.9 local BS canonical cache repair skipped", error);
    return false;
  }
}
...
async function pullLatestSnapshot(forceBootstrap = false) {
  ...
  if (data.unchanged) {
    ...
    saveCloudConfig(config);
    // V37.9: keep the proven main revision path independent from Promotion Light Sync.
  ...
  if (hasBsLegacyIdsV364(data.products)) {
    const repair = await callGoogleApi({
      ...
      bootstrapRevision:Number(data.revision)||0, updatedBy:"System V37.9 Stable"
    });
  ...
  // V37.9: a lightweight promotion response may have advanced this device while
...
function retryPendingMinimumPriceV345(){
  ...
  Promise.resolve(updateProductMinimumPriceFast(pending.productId,pending.minimumPrice,pending.updatedAt,pending.minimumPriceManual,0,pending.historyMetaV374||null))
    .catch(error=>console.warn("V37.9 pending minimum-price retry kept for next sync",error))
...
async function updateProductMinimumPriceFast(productId, minimumPrice, updatedAt, minimumPriceManual = true, retryCountV341 = 0, historyMetaV374 = null) {
  ...
  try {
    ...
    console.warn("V37.9 minimum-price local refresh skipped", localErrorV341);
  }

  setMinimumPricePendingV345(null);
  renderCloudMeta(config);
  setCloudState("synced");
  window.setTimeout(()=>{try{if(typeof refreshSystemViewsAfterSync==="function")refreshSystemViewsAfterSync()}catch(refreshErrorV343){console.warn("V37.9 deferred minimum-price refresh skipped",refreshErrorV343)}},0);
  return data;
}
...
async function reconcilePromotionWriteV374(requestedPromotion, attempts = 2) {
  for (let indexV373 = 0; indexV373 < attempts; indexV373 += 1) {
    ...
    try {
      ...
      console.warn("V37.9 promotion timeout reconcile skipped", errorV373);
    }
  }
  return null;
}
...
function getPromotionLightPollIntervalV375() {
  ...
  // V37.9 mobile is strictly read-only. Merely opening the Settings panel must not
...
async function pollPromotionStateLightV372(force = false) {
  ...
  try {
    ...
    if (changedV372 || promotionRevisionAdvancedV376) {
      ...
      } catch (error) { console.warn("V37.9 promotion light repaint skipped", error); }
    }
    ...
    console.warn("V37.9 promotion light sync skipped", error);
    return false;
  } finally {
...
async function pushPendingSnapshot(queue, retryCount = 0) {
  ...
  const data = await callGoogleApi({
    ...
    updatedBy:"System V37.9 Stable", collections,
...
async function updateInitialMinimumPriceFastV376(productId, initialMinimumPrice, retryCount=0){
  if(!isCloudWriteCredentialCurrentV341()) await pullLatestSnapshot(false);
  const config=getCloudConfig();
  const data=await callGoogleApi({action:"updateInitialMinimumPriceV376",clientVersion:APP_VERSION,schemaVersion:CLOUD_SCHEMA_VERSION,baseRevision:Number(config.revision)||0,bootstrapToken:String(config.bootstrapToken||""),bootstrapRevision:Number(config.bootstrapRevision)||0,updatedBy:"System V37.9 Stable",productId:String(productId||"").trim(),initialMinimumPrice:Number(initialMinimumPrice)});
...
async function restoreInitialMinimumPricesFastV376(retryCount=0){
  ...
  const data=await callGoogleApi({action:"restoreInitialMinimumPricesV376",clientVersion:APP_VERSION,schemaVersion:CLOUD_SCHEMA_VERSION,baseRevision:Number(config.revision)||0,bootstrapToken:String(config.bootstrapToken||""),bootstrapRevision:Number(config.bootstrapRevision)||0,updatedBy:"System V37.9 Stable"});
