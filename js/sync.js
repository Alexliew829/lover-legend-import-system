...
async function commitSalesCorrectionBatchToCloudV110(payload) {
  ...
  try { const data=await callGoogleApi({action:"commitSalesCorrectionBatchV110",clientVersion:APP_VERSION,schemaVersion:CLOUD_SCHEMA_VERSION,baseRevision:Number(config.revision)||0,bootstrapToken:String(config.bootstrapToken||""),bootstrapRevision:Number(config.bootstrapRevision)||0,updatedBy:"System V16.3 Stable",...payload});
...
async function migrateProductPrefixesV163() {
  await flushCloudQueueStrictV83();
  const config = getCloudConfig();
  const data = await callGoogleApi({
    action: "migrateProductPrefixesV163", clientVersion: APP_VERSION,
...
window.migrateProductPrefixesV163 = migrateProductPrefixesV163;
