...
async function commitSalesCorrectionBatchToCloudV110(payload) {
  ...
  try { const data=await callGoogleApi({action:"commitSalesCorrectionBatchV110",clientVersion:APP_VERSION,schemaVersion:CLOUD_SCHEMA_VERSION,baseRevision:Number(config.revision)||0,bootstrapToken:String(config.bootstrapToken||""),bootstrapRevision:Number(config.bootstrapRevision)||0,updatedBy:"System V29.8 Stable",...payload});
...
function refreshSystemViewsAfterSync() {
  const accessLockV297 = document.getElementById("accessLock");
  if (accessLockV297 && !accessLockV297.hidden) {
    window.__cloudRefreshDeferredV297 = true;
    renderCloudMeta(getCloudConfig());
    return;
  }
  [
