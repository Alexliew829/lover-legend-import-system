...
async function commitSalesCorrectionBatchToCloudV110(payload) {
  ...
  try { const data=await callGoogleApi({action:"commitSalesCorrectionBatchV110",clientVersion:APP_VERSION,schemaVersion:CLOUD_SCHEMA_VERSION,baseRevision:Number(config.revision)||0,bootstrapToken:String(config.bootstrapToken||""),bootstrapRevision:Number(config.bootstrapRevision)||0,updatedBy:"System V40.0 Stable",...payload});
...
function repairLocalBsCanonicalCacheV365() {
  try {
    ...
    console.warn("V40.0 local BS canonical cache repair skipped", error);
    return false;
  }
}
...
async function pullLatestSnapshot(forceBootstrap = false) {
  ...
  if (data.unchanged) {
    ...
    saveCloudConfig(config);
    // V40.0: keep the proven main revision path independent from Promotion Light Sync.
  ...
  if (hasBsLegacyIdsV364(data.products)) {
    const repair = await callGoogleApi({
      ...
      bootstrapRevision:Number(data.revision)||0, updatedBy:"System V40.0 Stable"
    });
  ...
  // V40.0: a lightweight promotion response may have advanced this device while
...
function retryPendingMinimumPriceV345(){
  ...
  Promise.resolve(updateProductMinimumPriceFast(pending.productId,pending.minimumPrice,pending.updatedAt,pending.minimumPriceManual,0,pending.historyMetaV374||null))
    .catch(error=>console.warn("V40.0 pending minimum-price retry kept for next sync",error))
...
async function updateProductMinimumPriceFast(productId, minimumPrice, updatedAt, minimumPriceManual = true, retryCountV341 = 0, historyMetaV374 = null) {
  ...
  try {
    ...
    // V40.0 root-cause fix: Number(null)===0. V38.1 therefore overwrote a valid
    ...
    console.warn("V40.0 minimum-price local refresh skipped", localErrorV341);
  }

  setMinimumPricePendingV345(null);
  renderCloudMeta(config);
  setCloudState("synced");
  window.setTimeout(()=>{try{if(typeof refreshSystemViewsAfterSync==="function")refreshSystemViewsAfterSync()}catch(refreshErrorV343){console.warn("V40.0 deferred minimum-price refresh skipped",refreshErrorV343)}},0);
  return data;
}
...
async function reconcilePromotionWriteV374(requestedPromotion, attempts = 2) {
  for (let indexV373 = 0; indexV373 < attempts; indexV373 += 1) {
    ...
    try {
      ...
      console.warn("V40.0 promotion timeout reconcile skipped", errorV373);
    }
  }
  return null;
}
...
async function updatePromotionSettingsFastV185(promotion, retryCountV374 = 0, forceFreshCredentialV398 = false) {
  ...
  setCloudState("syncing");
  try {
    // V40.0: normal Promotion writes use the already-valid bootstrap credential first.
    ...
    if (/timeout|connection failed/i.test(messageV374)) {
      // V40.0: a timeout may mean the server already committed the write.
...
function getPromotionLightPollIntervalV375() {
  ...
  // V40.0 mobile promotion control uses the same proven Promotion Light Sync path
...
async function pollPromotionStateLightV372(force = false) {
  ...
  try {
    ...
    // V40.0: if a Start/Update/Close began after this poll was sent, this response
    ...
    if (changedV372 || promotionRevisionAdvancedV376) {
      ...
      } catch (error) { console.warn("V40.0 promotion light repaint skipped", error); }
    }
    ...
    console.warn("V40.0 promotion light sync skipped", error);
    return false;
  } finally {
...
async function pushPendingSnapshot(queue, retryCount = 0) {
  ...
  const data = await callGoogleApi({
    ...
    updatedBy:"System V40.0 Stable", collections,
...
function sanitizeLegacySettingsV323(settings = {}, products = []) {
  ...
  // V40.0: promotion remains one live Settings key, now containing up to five packages.
...
function mergeSnapshots(remote, local, queue) {
  ...
  // V40.0: dedicated cloud writers own these live pricing/promotion keys. During a
...
async function updateInitialMinimumPriceFastV376(productId, initialMinimumPrice, retryCount=0){
  if(!isCloudWriteCredentialCurrentV341()) await pullLatestSnapshot(false);
  const config=getCloudConfig();
  const data=await callGoogleApi({action:"updateInitialMinimumPriceV376",clientVersion:APP_VERSION,schemaVersion:CLOUD_SCHEMA_VERSION,baseRevision:Number(config.revision)||0,bootstrapToken:String(config.bootstrapToken||""),bootstrapRevision:Number(config.bootstrapRevision)||0,updatedBy:"System V40.0 Stable",productId:String(productId||"").trim(),initialMinimumPrice:Number(initialMinimumPrice)});
...
async function restoreInitialMinimumPricesFastV376(retryCount=0){
  ...
  const data=await callGoogleApi({action:"restoreInitialMinimumPricesV376",clientVersion:APP_VERSION,schemaVersion:CLOUD_SCHEMA_VERSION,baseRevision:Number(config.revision)||0,bootstrapToken:String(config.bootstrapToken||""),bootstrapRevision:Number(config.bootstrapRevision)||0,updatedBy:"System V40.0 Stable"});
