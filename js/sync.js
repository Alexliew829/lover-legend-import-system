...
// V33.9: remember whether a real unsynced queue already existed before any
...
function setupCloudSync() {
  ...
  // V33.9 Local-First: when a valid V32.5-style bootstrap and cached core data
...
async function commitSalesCorrectionBatchToCloudV110(payload) {
  ...
  try { const data=await callGoogleApi({action:"commitSalesCorrectionBatchV110",clientVersion:APP_VERSION,schemaVersion:CLOUD_SCHEMA_VERSION,baseRevision:Number(config.revision)||0,bootstrapToken:String(config.bootstrapToken||""),bootstrapRevision:Number(config.bootstrapRevision)||0,updatedBy:"System V33.9 Stable",...payload});
...
async function runCloudSync() {
  ...
  try {
    ...
    // V33.9: if the queue was clean before this page loaded but became dirty while
    ...
    // V33.9: after one successful bootstrap, read-only revision checks stay in the
...
function applyRemoteData(data) {
  ...
  try {
    // V33.9: keep the proven V32.5 Pull/apply path simple.
    ...
    // V33.9: direct cloud writes must invalidate the prepared inventory join.
  ...
  // V33.9: a cloud Pull is read-only. Do not auto-repair/write cost snapshots
