...
// V35.3: remember whether a real unsynced queue already existed before any
...
function setupCloudSync() {
  ...
  // V35.3 Local-First: when a valid V32.5-style bootstrap and cached core data
...
async function commitSalesCorrectionBatchToCloudV110(payload) {
  ...
  try { const data=await callGoogleApi({action:"commitSalesCorrectionBatchV110",clientVersion:APP_VERSION,schemaVersion:CLOUD_SCHEMA_VERSION,baseRevision:Number(config.revision)||0,bootstrapToken:String(config.bootstrapToken||""),bootstrapRevision:Number(config.bootstrapRevision)||0,updatedBy:"System V35.3 Stable",...payload});
...
async function runCloudSync() {
  ...
  try {
    ...
    // V35.3: if the queue was clean before this page loaded but became dirty while
    ...
    // V35.3: after one successful bootstrap, read-only revision checks stay in the
    ...
    // V35.3: a bootstrap record from an older cached frontend may be complete but
...
async function pullLatestSnapshot(forceBootstrap = false) {
  ...
  if (data.unchanged) {
    ...
    saveCloudConfig(config);
    // V35.3: even an unchanged Pull can return a fresh bootstrap token. Persist
...
function retryPendingMinimumPriceV345(){
  ...
  Promise.resolve(updateProductMinimumPriceFast(pending.productId,pending.minimumPrice,pending.updatedAt,pending.minimumPriceManual))
    .catch(error=>console.warn("V35.3 pending minimum-price retry kept for next sync",error))
...
async function updateProductMinimumPriceFast(productId, minimumPrice, updatedAt, minimumPriceManual = true, retryCountV341 = 0) {
  ...
  // V35.3: keep this single-product change authoritative locally as well.
  ...
  try {
    ...
    console.warn("V35.3 minimum-price local refresh skipped", localErrorV341);
  }

  setMinimumPricePendingV345(null);
  renderCloudMeta(config);
  setCloudState("synced");
  window.setTimeout(()=>{try{if(typeof refreshSystemViewsAfterSync==="function")refreshSystemViewsAfterSync()}catch(refreshErrorV343){console.warn("V35.3 deferred minimum-price refresh skipped",refreshErrorV343)}},0);
  return data;
}
...
async function updateMinimumPriceBatchV353(changes, retryCountV353 = 0) {
  ...
  // V35.3: a conflict-refresh may have reloaded the pre-change cloud snapshot.
  ...
  try {
    ...
    console.warn("V35.3 batch minimum-price local refresh skipped", localErrorV353);
  }
...
async function updatePromotionSettingsFastV185(promotion) {
  ...
  if (data.conflict) {
    // V35.3: preserve the cloud's currently active promotion. A failed update is
    ...
    config.bootstrapRevision = Number(data.revision) || Number(config.bootstrapRevision) || 0;
    saveCloudConfig(config);
    try { await pullLatestSnapshot(false); } catch (pullErrorV350) { console.warn("V35.3 promotion conflict refresh skipped", pullErrorV350); }
...
async function pushPendingSnapshot(queue, retryCount = 0) {
  ...
  const data = await callGoogleApi({
    ...
    updatedBy:"System V35.3 Stable", collections,
...
function sanitizeLegacySettingsV323(settings = {}, products = []) {
  ...
  // V35.3: promotion is live business state, not legacy residue. Preserve a valid
...
function applyRemoteData(data) {
  ...
  try {
    // V35.3: keep the proven V32.5 Pull/apply path, but promotionV183 is now
    ...
    // V35.3: direct cloud writes must invalidate the prepared inventory join.
  ...
  // V35.3: a cloud Pull is read-only. Do not auto-repair/write cost snapshots
