...
async function executeSalesInventoryCardBatchV125(lines) {
  ...
  if (typeof commitSalesInventoryBatchToCloudV125 !== "function") {
    throw new Error("V16.1 整张销售卡批量库存模块未载入，请强制刷新网页后再试。");
  }
  ...
  try {
    ...
    // V16.1: the batch endpoint has already flushed and verified Products,
...
function setupImportHistory() {
  ...
  button?.addEventListener("click", () => {
    // V16.1: normalize both visible date fields at click time.  Either field
...
function getDailyStockAdjustments(selectedDate, keyword = "") {
  ...
  // V16.1: restore the proven V15.0 date-query return contract.
  return getProducts()
    .flatMap(product =>
...
function getHistoryNetSoldLots(options = {}) {
  ...
  const relevant = getHistoryRelevantAdjustments(lookupOptions)
    .filter(adjustment => {
      ...
      // V16.1: retain positive changes only as possible reversals. Explicit
  ...
  relevant.forEach(adjustment => {
    ...
    if (!hasExplicitRestoreLink(adjustment)) {
      // V16.1: an unlinked +N is a test/manual undo only when it exactly
...
// V16.1: sum Sales-card profit and complete Sales-card cost for the exact
...
// V16.1: source lookup uses surviving net-sale lots. Cancelled or restored
...
// V16.1: 一次扫描 History，同时建立售出数量、累计利润及最近售出索引。
...
function renderInventoryManagementList() {
  ...
  // V16.1: both views consume this same sorted and filtered product array.
...
function getOriginalCostSummaryRows() {
  // V16.1: these are the actual objects just rendered by Inventory Management.
...
// V16.1 Shared quick navigation.  It deliberately observes only page/result
