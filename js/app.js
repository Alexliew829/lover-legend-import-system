...
function repairLegacyImportDates() {
  ...
  batches.forEach(batch => {
    ...
    // V5.6: keep V5.6 data model unchanged. Legacy/non-array items are
...
function setupImportModule(){
  ...
  const notifyBatchLookupNotFoundOnce = () => {
    ...
    // V5.6: do not refocus/select the field after the alert.
  ...
  // V5.6: change / blur only auto-load when the typed value is an exact
...
function loadBatchByNumber() {
  ...
  if (!batch) {
    ...
    if (partialMatches.length === 1) {
      ...
      // V5.6: do not force focus/select after closing the alert.
  ...
  if (!batch) {
    alert("找不到这个进口编号或海外运输单号。");
    // V5.6: never force focus/select here. On iPhone this previously caused
  ...
  // V5.6: never reconstruct inland cost from total cost.
  ...
  // V5.6: old batches without explicit inland cost stay empty.
...
function repairInventoryConsistencyForProduct(productId) {
  ...
  // V5.6: repair only the stale lot-level remaining quantities. Products is the truth
...
function getHistorySoldAdjustmentUnitCost(adjustment) {
  // V5.6: new sale records lock the unit cost at the moment of sale.
...
function getHistoryNetSoldLots(options = {}) {
  // V5.6: first resolve sale/correction pairs against COMPLETE history, then apply
...
// V5.6 History transaction-date rule:
...
function renderImportHistory() {
  ...
  // V5.6 History lookup safety: resolve the typed product against the current
  ...
  // V5.6: product-name History uses Products.stock as the final truth for total remaining.
...
function calculateBatch() {
  ...
  // V5.6 mapping field: inland miscellaneous cost is the two explicit
  ...
  // V5.6: inventory movement is NOT an import-cost recalculation.
...
function saveBatchImport() {
  ...
  if (isEditing) {
    ...
    // V5.6: ordinary metadata updates always save. Cost fields only update when
  ...
  const batch = {
    ...
    trackingNumber: document.getElementById("batchTrackingNumber").value.trim(),
    overseasTrackingNumber: document.getElementById("batchOverseasTrackingNumber").value.trim(),
    // V5.6: batch costs only come from current input. Never inherit from previous currency/product.
    ...
    potRM: 0,
    // V5.6: explicit mapping fields for Pricing Suite.
...
function bindProductStockLongPress() {
  // V5.6 safety mode: follow the proven V4.20 interaction.
...
function saveInventoryConsistencySnapshot(previousProducts, nextProducts, previousImports, nextImports, previousBatches, nextBatches) {
  // V5.6: write the three related collections to localStorage first, then mark one sync snapshot.
...
async function editProductMinimumPrice(productId) {
  ...
  // V5.6 fast path: save one local Products value without marking
...
function getStaleZeroStockProducts() {
  // V5.6: user-confirmed cleanup rule. Every product whose canonical
...
function deleteSelectedStaleZeroStockProducts() {
  ...
  // V5.6 atomic local snapshot: write all related collections first, then mark one cloud sync set.
  localStorage.setItem("importSystemProducts", JSON.stringify(nextProducts));
  localStorage.setItem("importSystemImports", JSON.stringify(nextImports));
  localStorage.setItem("importSystemBatches", JSON.stringify(nextBatches));
