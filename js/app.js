...
const PREMIUM_LIBRARY_KEY = "importSystemPremiumLibrary";

function getPremiumItems() {
  const items = loadJSON(PREMIUM_LIBRARY_KEY, []);
  return Array.isArray(items) ? items : [];
}

function savePremiumItems(items) {
  const previous = getPremiumItems();
  saveJSON(PREMIUM_LIBRARY_KEY, items);

  if (typeof markCloudCollectionSaved === "function") {
    markCloudCollectionSaved(
      "premiumItems",
      previous,
      items
    );
...
function setupPremiumLibrary() {
  ...
  nameInput?.addEventListener("input", () => {
    const chars = Array.from(nameInput.value || "");
    if (chars.length > 12) {
      nameInput.value = chars.slice(0, 12).join("");
    }
    ...
    if (counter) {
      counter.textContent =
        `${Array.from(nameInput.value || "").length} / 12`;
    }
  });
  ...
  document
    .getElementById("premiumNewBtn")
    ?.addEventListener("click", resetPremiumForm);

  document
    .getElementById("premiumResetBtn")
    ?.addEventListener("click", resetPremiumForm);
  ...
  document
    .getElementById("premiumList")
    ?.addEventListener("click", handlePremiumListClick);

  resetPremiumForm();
  renderPremiumLibrary();
}

function getPremiumFormValues() {
  return {
    name: String(
      document.getElementById("premiumName")?.value || ""
    ).trim(),
    crownSpread: String(
      document.getElementById("premiumCrownSpread")?.value || ""
    ).trim(),
    width: String(
      document.getElementById("premiumWidth")?.value || ""
    ).trim(),
    heightWithPot: String(
      document.getElementById("premiumHeightWithPot")?.value || ""
    ).trim(),
    trunkDiameter: String(
      document.getElementById("premiumTrunkDiameter")?.value || ""
    ).trim(),
    rootBase: String(
      document.getElementById("premiumRootBase")?.value || ""
    ).trim(),
    potDiameter: String(
      document.getElementById("premiumPotDiameter")?.value || ""
    ).trim(),
    price: String(
      document.getElementById("premiumPrice")?.value || ""
    ).trim(),
    videoUrl: String(
      document.getElementById("premiumVideoUrl")?.value || ""
    ).trim()
  };
}

function savePremiumItem() {
  const items = getPremiumItems();
  const editingId = String(
    document.getElementById("premiumEditingId")?.value || ""
  ).trim();

  const values = getPremiumFormValues();

  if (Array.from(values.name).length > 12) {
    showPremiumStatus("产品名称最多12个字", true);
    return;
  }
  ...
  if (editingId) {
    const index = items.findIndex(
      item => String(item.id || "") === editingId
    );

    if (index === -1) {
      showPremiumStatus("找不到这项精品资料", true);
      return;
    }
    ...
    savePremiumItems(items);
    showPremiumStatus("已更新精品资料");
  } else {
    items.unshift({
      id:
        `PRM${Date.now()}${Math.random()
          .toString(36)
          .slice(2, 6)}`,
      ...values,
      createdAt: now,
    ...
    savePremiumItems(items);
    showPremiumStatus("已保存精品资料");
  }

  resetPremiumForm(false);
  renderPremiumLibrary();
}

function resetPremiumForm(clearStatus = true) {
  const ids = [
    "premiumEditingId",
    "premiumName",
    "premiumCrownSpread",
    "premiumWidth",
    "premiumHeightWithPot",
    "premiumTrunkDiameter",
    "premiumRootBase",
    "premiumPotDiameter",
    "premiumPrice",
    "premiumVideoUrl"
  ...
  const counter =
    document.getElementById("premiumNameCounter");
  if (counter) counter.textContent = "0 / 12";

  const saveButton =
    document.getElementById("premiumSaveBtn");
  if (saveButton) saveButton.textContent = "保存精品资料";

  if (clearStatus) {
    const status =
      document.getElementById("premiumFormStatus");
    if (status) status.textContent = "";
  }
}
...
function formatPremiumValue(value, suffix = "") {
  const text = String(value || "").trim();
  return text ? `${text}${suffix}` : "";
}

function formatPremiumPrice(value) {
  const text = String(value || "").trim();
  if (!text) return "";
...
function buildPremiumDescriptionLines(item) {
  const rows = [
    [
      "冠幅 Crown Spread",
      formatPremiumValue(item.crownSpread, " CM")
    ],
    [
      "宽度 Width",
      formatPremiumValue(item.width, " CM")
    ],
    [
      "含盆高度 Height incl. Pot",
      formatPremiumValue(item.heightWithPot, " CM")
    ],
    [
      "主干直径 Trunk Diameter",
      formatPremiumValue(item.trunkDiameter, " CM")
    ],
    [
      "根盘 Root Base",
      formatPremiumValue(item.rootBase, " CM")
    ],
    [
      "花盆外径 Pot Diameter",
      formatPremiumValue(item.potDiameter, " CM")
    ],
    [
      "售价 Price",
      formatPremiumPrice(item.price)
    ]
  ];
...
function buildPremiumCopyText(item) {
  const lines = [];
  ...
  const descriptions =
    buildPremiumDescriptionLines(item);

  if (descriptions.length && lines.length) {
    lines.push("");
  }
...
async function handlePremiumListClick(event) {
  const button = event.target.closest(
    "[data-premium-action]"
  );
  if (!button) return;
  ...
  const id = String(
    button.dataset.premiumId || ""
  );
  const action =
    button.dataset.premiumAction;
  const item = getPremiumItems().find(
  ...
  if (action === "copy-details") {
    const text = buildPremiumCopyText(item);

    if (!text) {
      showPremiumStatus("没有可复制的名称或描述", true);
      return;
    }

    const copied = await copyTextToClipboard(text);
    showPremiumStatus(
      copied ? "已复制名称和产品描述" : "复制失败",
      !copied
    );
    return;
  }
  ...
  if (action === "edit") {
    editPremiumItem(item);
    return;
  }

  if (action === "delete") {
    deletePremiumItem(item);
  }
}

function editPremiumItem(item) {
  const values = {
    premiumEditingId: item.id || "",
    premiumName: item.name || "",
    premiumCrownSpread: item.crownSpread || "",
    premiumWidth: item.width || "",
    premiumHeightWithPot: item.heightWithPot || "",
    premiumTrunkDiameter: item.trunkDiameter || "",
    premiumRootBase: item.rootBase || "",
    premiumPotDiameter: item.potDiameter || "",
    premiumPrice: item.price || "",
    premiumVideoUrl: item.videoUrl || ""
  };
  ...
  if (counter) {
    counter.textContent =
      `${Array.from(item.name || "").length} / 12`;
  }

  const saveButton =
    document.getElementById("premiumSaveBtn");
  if (saveButton) saveButton.textContent = "更新精品资料";

  showPremiumStatus("正在编辑精品资料");
  window.scrollTo({ top: 0, behavior: "smooth" });
}

function deletePremiumItem(item) {
  const label =
    String(item.name || "").trim() || "未命名精品";
...
function renderPremiumLibrary() {
  ...
  const keyword = String(
    document.getElementById("premiumSearch")?.value || ""
  ).trim();

  const items = getPremiumItems()
    .filter(item =>
      smartSearchMatches(item.name || "", keyword)
    )
    .sort((a, b) =>
  ...
  count.textContent = `${formatNumber(items.length)} 项`;

  if (!items.length) {
    list.innerHTML =
      '<div class="empty-state">暂无精品资料</div>';
    return;
  }
  ...
  list.innerHTML = items.map(item => {
    ...
    const descriptionHtml = descriptions.length
      ? descriptions.map(([label, value]) => `
          <div class="premium-description-row">
            <span>${escapeHTML(label)} ：</span>
            <strong>${escapeHTML(value)}</strong>
          </div>
        `).join("")
      : '<div class="premium-empty-description">尚未填写产品描述</div>';
    ...
    return `
      <article class="premium-card">
        <div class="premium-card-head">
          <h3>${escapeHTML(item.name || "未命名精品")}</h3>
          <span>${escapeHTML(
            formatDateDDMMYYYY(
        ...
        <div class="premium-card-actions">
          <button type="button"
                  ...
                  data-premium-action="copy-details"
                  data-premium-id="${escapeHTML(item.id)}">
            复制名称和描述
          </button>

          <button type="button"
                  class="premium-video-btn"
                  data-premium-action="copy-video"
                  data-premium-id="${escapeHTML(item.id)}"
                  ${videoUrl ? "" : "disabled"}>
            复制视频
          </button>
          ...
          <button type="button"
                  class="premium-edit-btn"
                  data-premium-action="edit"
                  data-premium-id="${escapeHTML(item.id)}">
            编辑
          </button>
...
window.renderPremiumLibrary = renderPremiumLibrary;
