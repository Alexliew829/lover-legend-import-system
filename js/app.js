...
function repairLegacyImportDates() {
  ...
  batches.forEach(batch => {
    ...
    // V4.24 compatibility guard:
    // Old batch data may contain items as a JSON string instead of an Array.
    // Parse only when it is a valid array; otherwise skip safely so startup
    // cannot be interrupted. This does not alter inventory/cost calculations.
    let batchItems = [];

    if (Array.isArray(batch.items)) {
      batchItems = batch.items;
    } else if (typeof batch.items === "string") {
      try {
        const parsedItems = JSON.parse(batch.items);
        if (Array.isArray(parsedItems)) {
          batchItems = parsedItems;
          batch.items = parsedItems;
          changed = true;
        }
      } catch (error) {
        console.warn(
          `Batch ${batch.importNumber || batch.id || "-"} items parse skipped:`,
          error
        );
      }
    }
...
function setupImportModule(){
  ...
  const notifyBatchLookupNotFoundOnce = () => {
    ...
    // V4.24: do not refocus/select the field after the alert.
  ...
  // V4.24: change / blur only auto-load when the typed value is an exact
...
function loadBatchByNumber() {
  ...
  if (!batch) {
    ...
    if (partialMatches.length === 1) {
      ...
      // V4.24: do not force focus/select after closing the alert.
  ...
  if (!batch) {
    alert("找不到这个进口编号或海外运输单号。");
    // V4.24: never force focus/select here. On iPhone this previously caused
  ...
  // V4.24: never reconstruct inland cost from total cost.
  ...
  // V4.24: old batches without explicit inland cost stay empty.
...
function calculateBatch() {
  ...
  // V4.24 mapping field: inland miscellaneous cost is the two explicit
  ...
  // V4.24: inventory movement is NOT an import-cost recalculation.
...
function saveBatchImport() {
  ...
  if (isEditing) {
    ...
    // V4.24: ordinary metadata updates always save. Cost fields only update when
  ...
  const batch = {
    ...
    trackingNumber: document.getElementById("batchTrackingNumber").value.trim(),
    overseasTrackingNumber: document.getElementById("batchOverseasTrackingNumber").value.trim(),
    // V4.24: batch costs only come from current input. Never inherit from previous currency/product.
    ...
    potRM: 0,
    // V4.24: explicit mapping fields for Pricing Suite.
...
function restoreSystemData(event) {
  ...
  reader.onload = () => {
    try {
      ...
      // V4.24 restore migration: keep old cost fields compatible.
