...
async function executeSalesInventoryCardBatchV125(lines) {
  ...
  if (typeof commitSalesInventoryBatchToCloudV125 !== "function") {
    throw new Error("V18.9 整张销售卡批量库存模块未载入，请强制刷新网页后再试。");
  }
  ...
  try {
    ...
    // V18.9: the batch endpoint has already flushed and verified Products,
...
function getPromotionDeliveryV183(cost, price, rules) {
  return getMinimumFreightTierV188(price, rules).amount;
}
...
function getPromotionPriceBreakdownV183(product, promotion = null, rules = null, originIndex = null) {
  ...
  const priced = calculateTieredMinimumPriceV188(cost, denominator, configuredRules);
  const profit = priced.price * (1 - active.commissionRate / 100) - cost - priced.delivery;
  return { price:priced.price, cost, delivery:priced.delivery, tier:priced.tier, profit, profitRate:priced.price > 0 ? profit / priced.price * 100 : 0 };
}
...
function renderPromotionPriceListV183() {
  ...
  list.innerHTML = products.map(product => {
    ...
    return `<article class="promotion-price-row-v183 ${profit < 0 ? "loss" : "gain"} ${excluded ? "excluded" : ""}">
      <div class="promotion-product-copy-v183">
        ...
        ${excluded ? `<small class="promotion-excluded-note-v188">已排除 · 使用原最低售价</small>` : ""}
      </div>
      <div class="promotion-sales-stats-v188" title="按 Import History 的实际净售出数量计算">
        <span>售出数量<b>${formatNumber(product.netSoldQuantity || 0)}<small>棵</small></b></span>
        <span>累计利润<b>${formatMoney(product.cumulativeSoldProfit || 0, "RM ")}</b></span>
      </div>
      <span><small>平均成本</small><b>${formatMoney(product.averageCost, "RM ")}</b></span>
      <span><small>原最低售价</small><b>${formatMoney(originalPrice, "RM ")}</b></span>
      <span><small>促销最低售价</small><b>${formatMoney(effectivePrice, "RM ")}</b></span>
      <span class="promotion-profit-v188"><small>预计${profit < 0 ? "亏" : "赚"}</small><b>${formatMoney(Math.abs(profit), "RM ")}</b></span>
    </article>`;
...
function setupPromotionSettingsV183() {
  ...
  saveButton.addEventListener("click", async () => {
    ...
    if (saved) {
      [refreshPromotionUiV183, renderPromotionPriceListV183, renderDashboard, renderInventoryManagementList, renderProductList]
        .forEach(render => { try { render(); } catch (error) { console.warn("V18.9 post-save refresh skipped:", error); } });
      updatePromotionDraftStatusV186();
    }
  });
  deleteButton?.addEventListener("click", async () => {
    ...
    if (deleted) {
      ...
      if (commissionInput) commissionInput.value = "10";
      ...
      [renderPromotionExcludedListV183, refreshPromotionUiV183, renderDashboard, renderInventoryManagementList, renderProductList]
        .forEach(render => { try { render(); } catch (error) { console.warn("V18.9 post-delete refresh skipped:", error); } });
...
function getProductPrefixRulesV181() {
  ...
  additional.forEach(rule => {
    ...
    // V18.9 one-time removal of the user's test-only rule.
...
function setupImportHistory() {
  ...
  button?.addEventListener("click", () => {
    // V18.9: normalize both visible date fields at click time.  Either field
...
function getDailyStockAdjustments(selectedDate, keyword = "") {
  ...
  // V18.9: restore the proven V15.0 date-query return contract.
  return getProducts()
    .flatMap(product =>
...
function getHistoryNetSoldLots(options = {}) {
  ...
  // V18.9: old/manual stock repairs do not always carry a Sales restore link.
  ...
  const relevant = getHistoryRelevantAdjustments(lookupOptions)
    .filter(adjustment => {
      ...
      // V18.9: retain positive changes only as possible reversals. Explicit
  ...
  relevant.forEach(adjustment => {
    ...
    if (!hasExplicitRestoreLink(adjustment) && !isExplicitManualRestoreV180(adjustment)) {
      // V18.9: an unlinked +N is a test/manual undo only when it exactly
...
// V18.9: sum Sales-card profit and complete Sales-card cost for the exact
...
// V18.9: source lookup uses surviving net-sale lots. Cancelled or restored
...
// V18.9: 一次扫描 History，同时建立售出数量、累计利润及最近售出索引。
...
function renderInventoryManagementList() {
  ...
  // V18.9: both views consume this same sorted and filtered product array.
...
function getOriginalCostSummaryRows() {
  // V18.9: these are the actual objects just rendered by Inventory Management.
...
// V18.9 Shared quick navigation.  It deliberately observes only page/result
