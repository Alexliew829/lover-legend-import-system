...
async function executeSalesInventoryCardBatchV125(lines) {
  ...
  if (typeof commitSalesInventoryBatchToCloudV125 !== "function") {
    throw new Error("V19.0 整张销售卡批量库存模块未载入，请强制刷新网页后再试。");
  }
  ...
  try {
    ...
    // V19.0: the batch endpoint has already flushed and verified Products,
...
function renderPromotionPriceListV183() {
  ...
  list.classList.add("inventory-management-list", "promotion-inventory-list-v190");
  list.innerHTML = products.map(product => {
    ...
    return `<article class="inventory-manage-card promotion-inventory-card-v190 ${excluded ? "excluded" : ""}">
      <div class="inventory-manage-head promotion-inventory-head-v190">
        <div class="inventory-manage-primary-v174">
          <div class="inventory-product-title-row">
            <span class="product-identity-v167 inventory-product-identity-v167">
              <button class="inventory-product-name-copy" type="button" data-product-name="${escapeHTML(product.name)}" onclick="copyInventoryProductName(this)" title="点击复制产品名称">${escapeHTML(product.name)}</button>
              ${buildProductIdCopyButtonV166(product.id, "inventory-product-id-v166")}
            </span>
          </div>
          <div class="product-code inventory-product-code-v166 promotion-status-v190">${excluded ? "已排除 · 使用原最低售价" : "促销价格"}</div>
        </div>
      ...
      <div class="inventory-summary-grid promotion-summary-grid-v190">
        ...
        <div><span>促销最低售价</span><strong>${formatMoney(effectivePrice, "RM ")}</strong></div>
        <div class="promotion-profit-box-v190 ${profit < 0 ? "loss" : "gain"}"><span>预计${profit < 0 ? "亏" : "赚"}</span><strong>${formatMoney(Math.abs(profit), "RM ")}</strong></div>
      </div>
    </article>`;
  ...
  saveButton.addEventListener("click", async () => {
    ...
    if (saved) {
      [refreshPromotionUiV183, renderPromotionPriceListV183, renderDashboard, renderInventoryManagementList, renderProductList]
        .forEach(render => { try { render(); } catch (error) { console.warn("V19.0 post-save refresh skipped:", error); } });
      updatePromotionDraftStatusV186();
    }
  });
  deleteButton?.addEventListener("click", async () => {
    ...
    if (deleted) {
      ...
      [renderPromotionExcludedListV183, refreshPromotionUiV183, renderDashboard, renderInventoryManagementList, renderProductList]
        .forEach(render => { try { render(); } catch (error) { console.warn("V19.0 post-delete refresh skipped:", error); } });
...
function getProductPrefixRulesV181() {
  ...
  additional.forEach(rule => {
    ...
    // V19.0 one-time removal of the user's test-only rule.
...
function setupImportHistory() {
  ...
  button?.addEventListener("click", () => {
    // V19.0: normalize both visible date fields at click time.  Either field
...
function getDailyStockAdjustments(selectedDate, keyword = "") {
  ...
  // V19.0: restore the proven V15.0 date-query return contract.
  return getProducts()
    .flatMap(product =>
...
function getHistoryNetSoldLots(options = {}) {
  ...
  // V19.0: old/manual stock repairs do not always carry a Sales restore link.
  ...
  const relevant = getHistoryRelevantAdjustments(lookupOptions)
    .filter(adjustment => {
      ...
      // V19.0: retain positive changes only as possible reversals. Explicit
  ...
  relevant.forEach(adjustment => {
    ...
    if (!hasExplicitRestoreLink(adjustment) && !isExplicitManualRestoreV180(adjustment)) {
      // V19.0: an unlinked +N is a test/manual undo only when it exactly
...
// V19.0: sum Sales-card profit and complete Sales-card cost for the exact
...
// V19.0: source lookup uses surviving net-sale lots. Cancelled or restored
...
// V19.0: 一次扫描 History，同时建立售出数量、累计利润及最近售出索引。
...
function renderInventoryManagementList() {
  ...
  // V19.0: both views consume this same sorted and filtered product array.
...
function getOriginalCostSummaryRows() {
  // V19.0: these are the actual objects just rendered by Inventory Management.
...
// V19.0 Shared quick navigation.  It deliberately observes only page/result
