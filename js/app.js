...
function setupImportModule(){
  ...
  const notifyBatchLookupNotFoundOnce = () => {
    ...
    // V4.21: do not refocus/select the field after the alert.
  ...
  // V4.21: change / blur only auto-load when the typed value is an exact
...
function loadBatchByNumber() {
  ...
  if (!batch) {
    ...
    if (partialMatches.length === 1) {
      ...
      // V4.21: do not force focus/select after closing the alert.
  ...
  if (!batch) {
    alert("找不到这个进口编号或海外运输单号。");
    // V4.21: never force focus/select here. On iPhone this previously caused
  ...
  // V4.21: never reconstruct inland cost from total cost.
  ...
  // V4.21: old batches without explicit inland cost stay empty.
...
function calculateBatch() {
  ...
  // V4.21 mapping field: inland miscellaneous cost is the two explicit
  ...
  // V4.21: inventory movement is NOT an import-cost recalculation.
...
function saveBatchImport() {
  ...
  if (isEditing) {
    ...
    // V4.21: ordinary metadata updates always save. Cost fields only update when
  ...
  const batch = {
    ...
    trackingNumber: document.getElementById("batchTrackingNumber").value.trim(),
    overseasTrackingNumber: document.getElementById("batchOverseasTrackingNumber").value.trim(),
    // V4.21: batch costs only come from current input. Never inherit from previous currency/product.
    ...
    potRM: 0,
    // V4.21: explicit mapping fields for Pricing Suite.
...
function restoreSystemData(event) {
  ...
  reader.onload = () => {
    try {
      ...
      // V4.21 restore migration: keep old cost fields compatible.
