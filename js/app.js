...
function restoreStoredBatchRMDisplay(batch, items) {
  ...
  const storedForeignRM = Number(
    batch.fixedTotalForeignCostsRM ??
    batch.totalForeignCostsRM
  );
  const storedShippingRate = Number(
    batch.fixedShippingRate ??
    batch.shippingRate
  );
  const storedGrandTotal = Number(
    batch.fixedGrandTotalRM ??
    batch.grandTotal
  );

  if (foreignRM && Number.isFinite(storedForeignRM)) {
    foreignRM.textContent =
      formatMoney(storedForeignRM || 0, "RM ");
  }

  if (shippingRate && Number.isFinite(storedShippingRate)) {
    shippingRate.textContent =
      `${formatMoney(storedShippingRate || 0)}%`;
  }

  if (grandTotal && Number.isFinite(storedGrandTotal)) {
    grandTotal.textContent =
      formatMoney(storedGrandTotal || 0, "RM ");
  }
...
function cleanupLegacyVndChinaTransportValuesV300() {
  // V5.6：永久停用旧版 VND 自动清零。
  // 保留空函数只是为了兼容旧调用，不修改任何历史费用。
  return 0;
}

function calculateFixedBatchCostSnapshot(batch, batchItems = []) {
  const currency = String(
    batch?.currency ||
    batchItems.find(item => item?.currency)?.currency ||
    "CNY"
  ).toUpperCase();

  const originalGoodsTotalFromItems =
  ...
  // 海外运费比例 = 海外到大马运费RM / 整批货款总额折合RM
  const fixedShippingRate =
    fixedShippingMY > 0 &&
    fixedTotalForeignCostsRM > 0
      ? fixedShippingMY /
        fixedTotalForeignCostsRM * 100
      : (
...
function ensureFixedCostSnapshotsV302() {
  ...
  const nextImports = imports.map(record => {
    const alreadyLocked =
      ...
      String(record?.costSnapshotVersion || "") === "5.6";
...
function publishPricingSuiteImportUnitPrices() {
  try {
    ...
    const addRecord = source => {
      ...
      // V5.6：Pricing Suite 优先读取进口保存时锁定的成本快照。
...
function normalizeBackupForV51(rawData) {
  if (!rawData || typeof rawData !== "object") {
    throw new Error("Backup不是有效JSON对象");
  }
  ...
  if (
    ...
    throw new Error("Backup缺少产品／进口资料");
  }
  ...
  data.imports.forEach(record => {
    ...
    keys.forEach(key => {
      if (!importsByBatch.has(key)) {
        importsByBatch.set(key, []);
      }
      importsByBatch.get(key).push(record);
    });
  });
  ...
  data.batches = data.batches.map(batch => {
    ...
    const sourceItems =
      ...
      batch.items.length
        ? batch.items
        : (
            importsByBatch.get(batchKey) ||
            importsByBatch.get(importKey) ||
            []
          );
    ...
    const currency = String(
      batch?.currency ||
      sourceItems.find(item => item?.currency)?.currency ||
      "CNY"
    ).toUpperCase();
    ...
    const nextBatch = {
      ...
      // legacy字段继续保留，保证旧UI/Apps Script兼容。
      chinaTransportCost:
        snapshot.fixedChinaTransportCost,
      potCost:
        snapshot.fixedPotCost,
      shippingMY:
        snapshot.fixedShippingMY,
      shippingRate:
        snapshot.fixedShippingRate,
      totalForeignCostsRM:
        snapshot.fixedTotalForeignCostsRM,
      grandTotal:
        snapshot.fixedGrandTotalRM,

      chinaTransportSource:
        currency === "VND"
          ? (chinaManual ? "manual" : "empty")
          : (
              snapshot.fixedChinaTransportCost > 0
                ? "manual"
                : String(batch?.chinaTransportSource || "empty")
            ),

      potCostSource:
        currency === "VND"
          ? (potManual ? "manual" : "empty")
          : (
              snapshot.fixedPotCost > 0
                ? "manual"
    ...
    if (Array.isArray(batch?.items)) {
      nextBatch.items = batch.items.map(item => ({
        ...item,
        ...buildFixedImportCostSnapshot(
          item,
          snapshot
  ...
  data.imports = data.imports.map(record => {
    const batch =
      byId.get(String(record?.batchId || "")) ||
      byNumber.get(
        String(record?.importNumber || "")
          .trim().toLowerCase()
      );
...
function restoreSystemData(event) {
  ...
  reader.onload = () => {
    try {
      ...
      const data =
        normalizeBackupForV51(parsed);

      const confirmed = confirm(
        "Restore 会覆盖当前产品、库存和进口记录。\\n\\n" +
        "旧版本 Backup 会自动升级为 V5.6 成本结构。\\n" +
        "确定继续？"
      );

      if (!confirmed) return;

      // 一次写完，避免Restore过程中半套资料触发同步。
      localStorage.setItem(
        "importSystemSettings",
        JSON.stringify(data.settings || {})
      );
      localStorage.setItem(
        "importSystemProducts",
        JSON.stringify(data.products)
      );
      localStorage.setItem(
        "importSystemImports",
        JSON.stringify(data.imports)
      );
      localStorage.setItem(
        "importSystemBatches",
      ...
      showDataToolsStatus(
        `Restore 已完成：${data.products.length} 个产品、` +
        `${data.imports.length} 笔进口明细、` +
        `${data.batches.length} 个进口编号`
      );
    } catch (error) {
