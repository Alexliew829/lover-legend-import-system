...
async function executeSalesInventoryCardBatchV125(lines) {
  ...
  if (typeof commitSalesInventoryBatchToCloudV125 !== "function") {
    throw new Error("V17.7 整张销售卡批量库存模块未载入，请强制刷新网页后再试。");
  }
  ...
  try {
    ...
    // V17.7: the batch endpoint has already flushed and verified Products,
...
function setupImportHistory() {
  ...
  button?.addEventListener("click", () => {
    // V17.7: normalize both visible date fields at click time.  Either field
...
function getDailyStockAdjustments(selectedDate, keyword = "") {
  ...
  // V17.7: restore the proven V15.0 date-query return contract.
  return getProducts()
    .flatMap(product =>
...
function getHistoryNetSoldLots(options = {}) {
  ...
  const relevant = getHistoryRelevantAdjustments(lookupOptions)
    .filter(adjustment => {
      ...
      // V17.7: retain positive changes only as possible reversals. Explicit
  ...
  relevant.forEach(adjustment => {
    ...
    if (!hasExplicitRestoreLink(adjustment)) {
      // V17.7: an unlinked +N is a test/manual undo only when it exactly
...
// V17.7: sum Sales-card profit and complete Sales-card cost for the exact
...
// V17.7: source lookup uses surviving net-sale lots. Cancelled or restored
...
// V17.7: 一次扫描 History，同时建立售出数量、累计利润及最近售出索引。
...
function renderInventoryManagementList() {
  ...
  // V17.7: both views consume this same sorted and filtered product array.
...
function getOriginalCostSummaryRows() {
  // V17.7: these are the actual objects just rendered by Inventory Management.
...
// V17.7 Shared quick navigation.  It deliberately observes only page/result
