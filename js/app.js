...
async function executeSalesInventoryCardBatchV125(lines) {
  ...
  if (typeof commitSalesInventoryBatchToCloudV125 !== "function") {
    throw new Error("V18.8 整张销售卡批量库存模块未载入，请强制刷新网页后再试。");
  }
  ...
  try {
    ...
    // V18.8: the batch endpoint has already flushed and verified Products,
...
function renderPromotionPriceListV183() {
  ...
  list.innerHTML = products.map(product => {
    ...
    return `<article class="promotion-price-row-v183 ${profit < 0 ? "loss" : "gain"} ${excluded ? "excluded" : ""}">
      ...
      <div class="promotion-sales-stats-v188">
        <span><small>售出数量</small><b>${formatNumber(product.netSoldQuantity || 0)}</b></span>
        <span><small>累计利润</small><b>${formatMoney(product.cumulativeSoldProfit || 0, "RM ")}</b></span>
      </div>
  ...
  saveButton.addEventListener("click", async () => {
    ...
    if (saved) {
      [refreshPromotionUiV183, renderPromotionPriceListV183, renderDashboard, renderInventoryManagementList, renderProductList]
        .forEach(render => { try { render(); } catch (error) { console.warn("V18.8 post-save refresh skipped:", error); } });
      updatePromotionDraftStatusV186();
    }
  });
  deleteButton?.addEventListener("click", async () => {
    ...
    if (deleted) {
      ...
      [renderPromotionExcludedListV183, refreshPromotionUiV183, renderDashboard, renderInventoryManagementList, renderProductList]
        .forEach(render => { try { render(); } catch (error) { console.warn("V18.8 post-delete refresh skipped:", error); } });
...
function getProductPrefixRulesV181() {
  ...
  additional.forEach(rule => {
    ...
    // V18.8 one-time removal of the user's test-only rule.
...
function setupImportHistory() {
  ...
  button?.addEventListener("click", () => {
    // V18.8: normalize both visible date fields at click time.  Either field
...
function getDailyStockAdjustments(selectedDate, keyword = "") {
  ...
  // V18.8: restore the proven V15.0 date-query return contract.
  return getProducts()
    .flatMap(product =>
...
function getHistoryNetSoldLots(options = {}) {
  ...
  // V18.8: old/manual stock repairs do not always carry a Sales restore link.
  ...
  const relevant = getHistoryRelevantAdjustments(lookupOptions)
    .filter(adjustment => {
      ...
      // V18.8: retain positive changes only as possible reversals. Explicit
  ...
  relevant.forEach(adjustment => {
    ...
    if (!hasExplicitRestoreLink(adjustment) && !isExplicitManualRestoreV180(adjustment)) {
      // V18.8: an unlinked +N is a test/manual undo only when it exactly
...
// V18.8: sum Sales-card profit and complete Sales-card cost for the exact
...
// V18.8: source lookup uses surviving net-sale lots. Cancelled or restored
...
// V18.8: 一次扫描 History，同时建立售出数量、累计利润及最近售出索引。
...
function renderInventoryManagementList() {
  ...
  // V18.8: both views consume this same sorted and filtered product array.
...
function getOriginalCostSummaryRows() {
  // V18.8: these are the actual objects just rendered by Inventory Management.
...
// V18.8 Shared quick navigation.  It deliberately observes only page/result
