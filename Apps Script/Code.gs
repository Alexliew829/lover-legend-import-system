...
function doPost(e) {
  try {
    ...
    if (body.action === "pull") return json_(pullAll_(body));
...
function initDatabase_() {
  ...
  PropertiesService.getScriptProperties().setProperty("revision", String(getSetting_("revision") || "0"));
...
function pullAll_(body) {
  ...
  const knownRevision = Number(body && body.knownRevision) || 0;
  const forceFull = Boolean(body && body.forceFull);
  const hasLocalData = Boolean(body && body.hasLocalData);

  // 只有客户端确认本地已有完整资料且版本相同，才跳过整表读取。
  if (!forceFull && hasLocalData && knownRevision === revision) {
    return { ok: true, unchanged: true, revision };
  }

  const products = readObjects_(SHEETS.PRODUCTS);
  const imports = readObjects_(SHEETS.IMPORTS);
  const batches = readBatches_();
  ...
  return {
    ...
    imports,
    batches,
    counts: { products: products.length, imports: imports.length, batches: batches.length }
  };
}
...
function pushAll_(body) {
  ...
  try {
    ...
    if (!body.force && Number(body.baseRevision) !== current) {
      const latest = pullAll_({ forceFull: true });
      return {
        ...latest,
        conflict: true,
...
function readObjects_(name) {
  ...
  return values
    ...
    .map(row =>
      Object.fromEntries(
        keys.map((key, index) => [key, serialize_(row[index], key)])
      )
    );
}
...
function writeObjects_(name, items) {
  ...
  if (newCount > 0) {
    const rows = items.map(item => keys.map(key => toSheetValue_(key, item[key])));
    sh.getRange(2, 1, newCount, keys.length).setValues(rows);
    applyDataFormats_(sh, schema, 2, newCount);
  }

  if (oldCount > newCount) {
    sh.getRange(newCount + 2, 1, oldCount - newCount, keys.length).clearContent();
  }
}
...
function toSheetValue_(key, value) {
  ...
  if (/^(date|lastImport|containerDate|arrivalDate)$/.test(key)) {
  ...
  if (/^(createdAt|updatedAt|timestamp)$/.test(key)) {
...
function serialize_(value, key) {
  if (value instanceof Date) {
    if (/^(createdAt|updatedAt|timestamp)$/.test(String(key || ""))) {
...
function getRevision_() {
  ...
  const cached = properties.getProperty("revision");
  if (cached !== null && cached !== "") return Number(cached) || 0;

  const revision = Number(getSetting_("revision") || 0);
  properties.setProperty("revision", String(revision));
  return revision;
}
  ...
  if (lastRow >= 2) {
    ...
    for (let index = 0; index < values.length; index += 1) {
      if (values[index][0] === key) {
        ...
        if (key === "revision") PropertiesService.getScriptProperties().setProperty("revision", String(value));
        return;
      }
    }
  }
...
function applyDataFormats_(sh, schema, startRow, rowCount) {
  if (!sh || rowCount < 1) return;

  schema.forEach(([key], index) => {
    const range = sh.getRange(startRow, index + 1, rowCount, 1);

    if (/^(date|lastImport|containerDate|arrivalDate)$/.test(key)) {
      range.setNumberFormat("dd-MM-yyyy");
    } else if (/^(createdAt|updatedAt|timestamp)$/.test(key)) {
      range.setNumberFormat("dd-MM-yyyy HH:mm:ss");
    } else if (/cost|price|rate|total|average|shipping|purchaseRM|grandTotal/i.test(key)) {
...
function formatAll_() {
  ...
  Object.entries(SCHEMAS).forEach(([name, schema]) => {
    ...
    applyDataFormats_(sh, schema, 2, dataRows);
    setColumnWidths_(sh, schema);
  });
}
